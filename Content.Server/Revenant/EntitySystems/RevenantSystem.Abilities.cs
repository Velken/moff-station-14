...
//using Content.Shared.Humanoid; //Moffstation - Revenatn Revamp
...
public sealed partial class RevenantSystem
{
    ...
    private void OnInteract(EntityUid uid, RevenantComponent component, UserActivateInWorldEvent args)
    {
        ...
        // Moffstation - Start - Revenant Revamp
        if (!_mobStateQuery.HasComp(target) || !HasComp<SiliconLawBoundComponent>(target) || HasComp<RevenantComponent>(target))
            return;
        // Moffstation - End - Revenant Revamp
